...
$(document).ready(async function () {
  ...
  async function generateResponse(input) {
    ...
    const prompt = `You are an AI assistant specializing in providing the most helpful advice for a programmer’s code. You will always have access to the programmer’s code in the input.
  
  Guidelines for Responses:
  Code Assistance: If the user requests code completion or an implementation, provide an explanation and a relevant code snippet. Ensure the snippet is contextually relevant to the code provided. If the request is unclear, ask for more details.
  
  Installation & General Questions: For queries about installations, packages, mathematical equations, or known algorithms, provide a well-worded explanation. Include code only if it enhances the response.
  
  Casual Conversation: Respond appropriately with good etiquette and ask what the user needs next
  
  Programmer's code:${sourceCode}
  ...
  require(["vs/editor/editor.main"], function (ignorable) {
    ...
    layout.registerComponent("assistant", function (container, state) {
      ...
      chatbox.insertBefore(apiKeyContainer, messages);

      container.getElement()[0].appendChild(chatbox);
