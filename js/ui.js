...
function setThemeMode(themeMode, saveToLocalStorage = false) {
  ...
  }.svg") center center / 33% no-repeat ${
    isLightMode ? "#e0e1e2" : "#1b1c1d"
  } `;
  ...
  [".ui.menu", ".ui.input"].forEach((selector) =>
  ...
  document
    ...
    .setAttribute(
      "data-content",
      `Switch between dark, light, and system theme (currently ${themeMode} theme)`
    );
...
document.addEventListener("DOMContentLoaded", function () {
  require(["vs/editor/editor.main"], function (ignorable) {
    ...
    setThemeMode(
      supportedThemeModes.includes(queryThemeMode)
        ? queryThemeMode
        : getThemeMode(),
      true
    );
